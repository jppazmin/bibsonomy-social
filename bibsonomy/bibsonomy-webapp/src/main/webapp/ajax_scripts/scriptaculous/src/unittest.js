...
}},assertMatch:function(c,f){var b=arguments[2]||"assertMatch";
var a=new RegExp(c);
try{(a.exec(f))?this.pass():this.fail(b+' : regex: "'+Test.Unit.inspect(c)+" did not match: "+Test.Unit.inspect(f)+'"')
...
}},assertReturnsTrue:function(f,d){var b=arguments[2]||"assertReturnsTrue";
try{var a=d[f];
if(!a){a=d["is"+f.charAt(0).toUpperCase()+f.slice(1)]
}a()?this.pass():this.fail(b+": method returned false")
}catch(c){this.error(c)
}},assertReturnsFalse:function(f,d){var b=arguments[2]||"assertReturnsFalse";
try{var a=d[f];
if(!a){a=d["is"+f.charAt(0).toUpperCase()+f.slice(1)]
...
Test.BDDMethods={};
for(m in METHODMAP){Test.BDDMethods[m]=eval("function(){"+"var args = $A(arguments);"+"var scope = args.shift();"+"scope."+METHODMAP[m]+".apply(scope,(args || []).concat([this])); }")
